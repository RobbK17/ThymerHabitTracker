...
**Version 1.1.0** · Habit Tracker collection plugin for [Thymer](https://thymer.com) (collection plugin, not a global plugin).
...
**1.1.0** — Plugin **`@version`** **1.1.0**. **Internal:** removed unused **`_getCategoryStreak`** / **`_getHabitStreak`** (the sidebar already uses **`_categoryStreakFromMap`** / **`_habitStreakFromMap`** with one **`_buildLogsByDateMap`** pass per render). **Stats view:** the stats **`renderContent`** path now uses **`_buildLogsByDateMap(records)`** instead of duplicating per-day log merge logic.

**1.0.10** — Plugin **`@version`** **1.0.10**. **Removed** the **Diagnose collection** command palette entry and **`_diagnose`** (record scan, write/read test, console + alert). **`onUnload`** removes **Manage**, **Refresh**, and **Delete empty log records** only.
