# ThymerHabitTracker
Habit Tracker Plugin for Thymer · **v1.1.0**  
_(COLLECTION Plugin; not global plugin)_

‼️ _In progress. Created by Claude and someone who knows nothing about coding! Suggestions and support very welcome!_ ‼️

### FEATURES
- Header Panel on Journal Page with Checkable Habits and Streak Displays
- Settings Screen to Configure Categories and Habits
- Category Streaks automatically agregated from habits within them
- Long press on habits with number counts to enter number, or single press to climb by one
- Stats menu with 30 day / 7 day calendar view
- Habit date automatically links with journal date but can be changed manually if desired
Weekly and Monthly activity reports (markdown)
Fixed ability to reorder activities via drag and drop
Performance improvements

